...
extension Board: GKGameModel {
  ...
  func setGameModel(_ gameModel: GKGameModel) {
    if let board = gameModel as? Board {
      //print("setGameModel: \(gameModel)")
      values = board.values
      //print("setGameModel values: \(values)")
  ...
  func apply(_ gameModelUpdate: GKGameModelUpdate) {
    guard  let move = gameModelUpdate as? Move else {
      print("gameModelUpdate: \(gameModelUpdate)")
      return
    }
  ...
  func score(for player: GKGameModelPlayer) -> Int {
    guard let player = player as? Player else {
      print("score: \(Move.Score.none.rawValue)")
      return Move.Score.none.rawValue
    }
    
    if isWin(for: player) {
      print("isWin: \(Move.Score.win.rawValue)")
